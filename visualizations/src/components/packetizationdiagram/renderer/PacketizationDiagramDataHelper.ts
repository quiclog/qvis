...
export default class PacketizationDiagramDataHelper {
    ...
    public static streamIDToColor(streamID:string, protocol:"HTTP2"|"HTTP3"):Array<string> {
        ...
        let lut;
        if ( protocol === "HTTP2" ) {
            lut = PacketizationDiagramDataHelper.H2streamIDColorLUT;
        }
        else {
            lut = PacketizationDiagramDataHelper.H3streamIDColorLUT;
        }
        ...
        if ( protocol === "HTTP2" ) {
            ...
            // FIXME: refactor this and the above properly and merge with the MultiplexingGraphDataHelper function
            if ( lut.size === 0 ){
